...
from games.bible_trivia.questions import get_random_bible_trivia_questions
from games.bible_trivia.bible_api import fetch_bible_verses, DEFAULT_COPYRIGHT

class BibleTriviaGame(BaseGame):
    ID = "bible_trivia"
    NAME = "Scripture Sprint: Bible Trivia"
    DESCRIPTION = "Challenging trivia across the Old & New Testament with verse verification, sliders, and multi-select!"
    ICON = "📜"
    MIN_PLAYERS = 1
    MAX_PLAYERS = 24
    ...
    def __init__(self, room_code: str, players: Dict[str, Dict[str, Any]], options: Optional[Dict[str, Any]] = None):
        ...
        self.question_deck = get_random_bible_trivia_questions(self.total_rounds, used_prompts=used_prompts)
        self.current_q_idx = 0
        self.current_q: Dict[str, Any] = {}
        self.current_verse_data: Dict[str, Any] = {}
    ...
    def _start_round(self) -> None:
        if self.current_q_idx < len(self.question_deck):
            ...
            self.current_q = dict(get_random_bible_trivia_questions(1)[0])

        verse_ref = self.current_q.get("verse_ref", "")
        self.current_verse_data = fetch_bible_verses(verse_ref)
    ...
    def advance_phase(self) -> bool:
        if self.stage == "answering":
            ...
            self.stage = "reveal"
            self.time_remaining = 8 # Extra time to read scripture on TV screen
            return True
    ...
    def get_host_view(self) -> Dict[str, Any]:
        ...
        return {
            ...
            "testament": self.current_q.get("testament", "Bible"),
            "category": self.current_q.get("category", "Scripture"),
            ...
            "verse_ref": self.current_q.get("verse_ref"),
            "verse_text": self.current_verse_data.get("text") if self.stage in ["reveal", "scoreboard", "game_over"] else None,
            "verse_url": self.current_verse_data.get("url"),
            "copyright": self.current_verse_data.get("copyright", DEFAULT_COPYRIGHT),
    ...
    def get_player_view(self, player_id: str) -> Dict[str, Any]:
        ...
        return {
            ...
            "correct_answer": self.current_q.get("answer") if self.stage in ["reveal", "scoreboard", "game_over"] else None,
            "verse_ref": self.current_q.get("verse_ref") if self.stage in ["reveal", "scoreboard", "game_over"] else None,
            "verse_text": self.current_verse_data.get("text") if self.stage in ["reveal", "scoreboard", "game_over"] else None,
            "verse_url": self.current_verse_data.get("url"),
            "copyright": self.current_verse_data.get("copyright", DEFAULT_COPYRIGHT),
